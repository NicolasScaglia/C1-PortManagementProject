...
public class Port {
    ...
    public String checkedCustoms(int weight, int hubNumber) {
        ...
        for(int i = 0; i < Hub.COLUMNS; i++){
            for(int j = 0; j < Hub.ROWS; j++){
                if(completePort[hubNumber - 1].dimensions[j][i] != null){
                    if(completePort[hubNumber - 1].dimensions[j][i].getWeight() <= weight){
                        container = completePort[hubNumber-1].dimensions[j][i];
                        container.inspected = true;
                        information.append(String.valueOf(container.ID)).append("\n").append(container.senderCompany).append("\n").append(String.valueOf(container.weight)).append("\nContainer has been inspected\n\n");
                    }
                }
            }
        }
